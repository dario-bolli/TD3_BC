...
def compute_reward_evaluation(prev_angle, angle, next_angle, force, target_corner, reward, hard_reward, max_actions, min_actions , perturbation=False):	#mean_actions, std_actions
    ...
    if perturbation:
        mean_pocket = 105.0736 - 5	#104.94109114059208 #105.0736 
        lbPocket = mean_pocket - 0.3106383	#0.9208495903864685	#0.3106383 #= 104.7629617
        ubPocket = mean_pocket + 0.2543617	#0.9208495903864685	#0.2543617 #= 105.3279617
        ...
        lbPocket = mean_pocket - 0.32  #0.9208495903864685	#0.3106383 #= 104.7629617
        ubPocket = mean_pocket + 0.36  #0.9208495903864685	#0.2543617 #= 105.3279617
        lbPocket_extended = mean_pocket - 0.9208495903864685
        ubPocket_extended = mean_pocket + 0.9208495903864685

    if reward != 1.0:
        if force > 0.1:
            if angle <= ubPocket_extended  and angle >= lbPocket_extended :     #Mathf.Max(currentTargetAngle, 
    ...
    return reward, hard_reward

def loss_2_behaviour(angle, cuedir, max_states, min_states, max_actions, min_actions ):	#, mean_actions, std_actions
	cuedir = cuedir*(max_states[18:21]-min_states[18:21]) + min_states[18:21]	#*std_states[18:21] + mean_states[18:21]
...
def min_max_standardize_features(dataset):
	
	max_s = dataset['states'].max(axis=0)
	min_s = dataset['states'].min(axis=0)
	dataset['states'] = (dataset['states'] - min_s)/(max_s - min_s)
	dataset['new_states'] = (dataset['new_states'] - min_s)/(max_s - min_s)

	max_a = dataset['actions'].max(axis=0)
	min_a = dataset['actions'].min(axis=0)
	dataset['actions'] = (dataset['actions'] - min_a)/(max_a - min_a)

	return max_s.values, min_s.values, max_a.values, min_a.values

def min_max_standardize_test_features(dataset, max_s, min_s, max_a, min_a):
...
def load_clean_data(filename):
	...
	#Train/Test split
	trial_clean = trial.unique()[0:75]	#[75:225]
	...
	### Perturbation set ##
	perturbation_trial_clean = trial.unique()[75:225]
	print(filename[0], "len trial: ", len(perturbation_trial_clean))
	perturbation_trial_ind = np.arange(0,len(perturbation_trial_clean))	#select only block 1,2.3 and 10 #1,len(trial.unique()) +1 if we want 250 index as well
	perturbation_train_trial_ind = np.random.choice(perturbation_trial_ind, size=int(0.8*len(perturbation_trial_ind)), replace=False)  #distrib proba for each value, could be useful to weight more "important" trajectories
	
	perturbation_test_trial_ind = np.delete(perturbation_trial_ind, perturbation_train_trial_ind)
	perturbation_train_trial = perturbation_trial_clean[perturbation_train_trial_ind]	
	perturbation_test_trial = perturbation_trial_clean[perturbation_test_trial_ind]

	perturbation_train_ind = trial.isin(perturbation_train_trial)
	perturbation_test_ind = trial.isin(perturbation_test_trial)

	perturbation_train_set = {'trial': trial[perturbation_train_ind], 	#to have indexes from 1 to 50 and not 26 to 76 (complex when adding other files)
					'states': states[perturbation_train_ind],
					'actions': actions[perturbation_train_ind],
					'new_states': new_states[perturbation_train_ind],
					'rewards': rewards[perturbation_train_ind],
					'terminals': terminals[perturbation_train_ind]}

	perturbation_test_set = {'trial': trial[perturbation_test_ind],
					'states': states[perturbation_test_ind],
					'actions': actions[perturbation_test_ind],
					'new_states': new_states[perturbation_test_ind],
					'rewards': rewards[perturbation_test_ind],
					'terminals': terminals[perturbation_test_ind]}
	...
	if len(filename) > 1:
		for i, file in enumerate(filename):
			if i > 0:
				...
				### Perturbation set ##
				perturbation_trial_clean = trial.unique()[75:225]
				print(file, "len trial: ", len(perturbation_trial_clean))
				perturbation_trial_ind = np.arange(0,len(perturbation_trial_clean))	#select only block 1,2.3 and 10 #1,len(trial.unique()) +1 if we want 250 index as well
				perturbation_train_trial_ind = np.random.choice(perturbation_trial_ind, size=int(0.8*len(perturbation_trial_ind)), replace=False)  #distrib proba for each value, could be useful to weight more "important" trajectories
				...
				perturbation_train_set['trial'] = pd.concat([perturbation_train_set['trial'], trial[perturbation_train_ind]+i*250], axis=0)
				perturbation_train_set['states'] = pd.concat([perturbation_train_set['states'], states[perturbation_train_ind]], axis=0)
				perturbation_train_set['actions'] = pd.concat([perturbation_train_set['actions'], actions[perturbation_train_ind]], axis=0)
				perturbation_train_set['new_states'] = pd.concat([perturbation_train_set['new_states'], new_states[perturbation_train_ind]], axis=0)
				perturbation_train_set['rewards'] = pd.concat([perturbation_train_set['rewards'], rewards[perturbation_train_ind]], axis=0)
				perturbation_train_set['terminals'] = pd.concat([perturbation_train_set['terminals'], terminals[perturbation_train_ind]], axis=0)	

				perturbation_test_set['trial'] = pd.concat([perturbation_test_set['trial'], trial[perturbation_test_ind]+i*250], axis=0)
				perturbation_test_set['states'] = pd.concat([perturbation_test_set['states'], states[perturbation_test_ind]], axis=0)
				perturbation_test_set['actions'] = pd.concat([perturbation_test_set['actions'], actions[perturbation_test_ind]], axis=0)
				perturbation_test_set['new_states'] = pd.concat([perturbation_test_set['new_states'], new_states[perturbation_test_ind]], axis=0)
				perturbation_test_set['rewards'] = pd.concat([perturbation_test_set['rewards'], rewards[perturbation_test_ind]], axis=0)
				perturbation_test_set['terminals'] = pd.concat([perturbation_test_set['terminals'], terminals[perturbation_test_ind]], axis=0)

	train_len = len(train_set['trial'])
	train_len2 = len(train_set['states'])
	perturbation_train_len = len(perturbation_train_set['trial'])
	print(train_len, train_len2, perturbation_train_len)
	test_len = len(test_set['trial'])
	perturbation_test_len = len(perturbation_test_set['trial'])
	print(test_len, perturbation_test_len)
	cat_train_set = {'trial': pd.concat([train_set['trial'], perturbation_train_set['trial']], axis=0),
					'states': pd.concat([train_set['states'], perturbation_train_set['states']], axis=0),
					'actions': pd.concat([train_set['actions'], perturbation_train_set['actions']], axis=0),
					'new_states': pd.concat([train_set['new_states'], perturbation_train_set['new_states']], axis=0),
					'rewards': pd.concat([train_set['rewards'], perturbation_train_set['rewards']], axis=0),
					'terminals': pd.concat([train_set['terminals'], perturbation_train_set['terminals']], axis=0)
	}

	cat_test_set = {'trial': pd.concat([test_set['trial'], perturbation_test_set['trial']], axis=0),
					'states': pd.concat([test_set['states'], perturbation_test_set['states']], axis=0),
					'actions': pd.concat([test_set['actions'], perturbation_test_set['actions']], axis=0),
					'new_states': pd.concat([test_set['new_states'], perturbation_test_set['new_states']], axis=0),
					'rewards': pd.concat([test_set['rewards'], perturbation_test_set['rewards']], axis=0),
					'terminals': pd.concat([test_set['terminals'], perturbation_test_set['terminals']], axis=0)
	}

	#mean_states, std_states, mean_actions, std_actions = normalize_features(cat_train_set)
	#normalize_test_features(cat_test_set, mean_states, std_states, mean_actions, std_actions)

	max_s, min_s, max_a, min_a = min_max_standardize_features(cat_train_set)
	print("min max values: ", max_s.shape, max_s,  min_s.shape, min_s, max_a.shape, max_a, min_a.shape, min_a)
	min_max_standardize_test_features(cat_test_set, max_s, min_s, max_a, min_a)

	norm_train_set =  {'trial': cat_train_set['trial'].iloc[0:train_len],
					'states': cat_train_set['states'].iloc[0:train_len],
					'actions': cat_train_set['actions'].iloc[0:train_len],
					'new_states': cat_train_set['new_states'].iloc[0:train_len],
					'rewards': cat_train_set['rewards'].iloc[0:train_len],
					'terminals': cat_train_set['terminals'].iloc[0:train_len]
					}
	
	norm_perturbation_train_set = {'trial': cat_train_set['trial'].iloc[train_len:],
					'states': cat_train_set['states'].iloc[train_len:],
					'actions': cat_train_set['actions'].iloc[train_len:],
					'new_states': cat_train_set['new_states'].iloc[train_len:],
					'rewards': cat_train_set['rewards'].iloc[train_len:],
					'terminals': cat_train_set['terminals'].iloc[train_len:]
					}
	
	norm_test_set =  {'trial': cat_test_set['trial'].iloc[0:test_len],
					'states': cat_test_set['states'].iloc[0:test_len],
					'actions': cat_test_set['actions'].iloc[0:test_len],
					'new_states': cat_test_set['new_states'].iloc[0:test_len],
					'rewards': cat_test_set['rewards'].iloc[0:test_len],
					'terminals': cat_test_set['terminals'].iloc[0:test_len]
					}
	 
	norm_perturbation_test_set = {'trial': cat_test_set['trial'].iloc[test_len:],
					'states': cat_test_set['states'].iloc[test_len:],
					'actions': cat_test_set['actions'].iloc[test_len:],
					'new_states': cat_test_set['new_states'].iloc[test_len:],
					'rewards': cat_test_set['rewards'].iloc[test_len:],
					'terminals': cat_test_set['terminals'].iloc[test_len:]
					}
	print(len(norm_train_set['trial']), len(norm_perturbation_train_set['trial']), len(norm_test_set['trial']), len(norm_perturbation_test_set['trial']))
	...
	return norm_train_set, norm_test_set, norm_perturbation_train_set, norm_perturbation_test_set, max_s, min_s, max_a, min_a	#mean_states, std_states, mean_actions, std_actions
...
class Actor(nn.Module):
	def __init__(self, state_dim, action_dim, hidden_dim, max_action):
		...
		self.l2 = nn.Linear(hidden_dim,hidden_dim)    # 512)
...
class Critic(nn.Module):
	def __init__(self, state_dim, action_dim, hidden_dim):
		...
		self.l6 = nn.Linear(hidden_dim,hidden_dim)    #512)
...
######################### Train ####################################
def train(filename, state_dim=21, action_dim=2, epochs=500000, epochs_warmup=500000, batch_size = 256, train=True, agent = "TD3_BC", data_folder = ""):  
	print("Load dataset")
	train_set, test_set, perturbation_train_set, perturbation_test_set, max_states, min_states, max_actions, min_actions = load_clean_data(filename)	#mean_states, std_states, mean_actions, std_actions
	...
	if agent == "CQL_SAC":
		...
		if train == True:
			...
			### Logs wandb	###
			wandb.init(project="reward-learning", name="5t_min-max_alpha-target5_lagrange10", config=args)
			# Magic
			...
			for i in tqdm(range(1, epochs_warmup)):
				...
				if (i%5000 == 0 and i<10000) or i%50000 == 0:
					#plot_action_animated_learnt_policy(train_set, policy, max_states, min_states, max_actions, min_actions, "CQL_SAC", data_folder)	#mean_states, std_states, mean_actions, std_actions
					#evaluation_loss = evaluate_close_to_human_behaviour(test_set, policy, max_states, min_states, max_actions, min_actions, "CQL_SAC", action_dim, data_folder)	##mean_states, std_states, mean_actions, std_actions
					#evaluation_losses = np.append(evaluation_losses, evaluation_loss.cpu().detach().numpy())
					policy.save('models/CQL_SAC'+data_folder+'_base/CQL_SAC_policy')
				...
				if (i%10 == 0 and i<500) or (i%50 == 0 and i<2000) or (i%200 == 0 and i<10000) or i%2000 == 0:
					...
					target_corner = states[3,9:12].cpu().detach().numpy()*(max_states[9:12]-min_states[9:12]) +	min_states[9:12]	#*std_states[9:12] + mean_states[9:12])		
					...
					action = action.cpu().detach().numpy()*(max_actions-min_actions) +	min_actions	#std_actions + mean_actions
					actions = actions.cpu().detach().numpy()*(max_actions-min_actions) + min_actions	#std_actions + mean_actions
					...
					avg_rewards, avg_hard_rewards, avg_rewards_human, avg_rewards_h, avg_hard_rewards_h, avg_losses_2_behaviour = evaluate_agent_performance(perturbation_test_set, policy,  max_states, min_states, max_actions, min_actions, agent)	#mean_states, std_states, mean_actions, std_actions,
					log_dict2 = {
					"AVG reward Agent": avg_rewards,
					"AVG hard reward Agent": avg_hard_rewards,
					"AVG reward Human control": avg_rewards_h,
					"AVG hard reward Human control": avg_hard_rewards_h,
					"AVG reward Human": avg_rewards_human,
					"AVG loss Agent-Human": avg_losses_2_behaviour,
					"Angle": angle_3,	
					"Force":force_3,
					"Angle_ref": angle_ref,
					"Force_ref": force_ref
			...
			'''for i in tqdm(range(1, epochs)):
				experience = weighted_sample_experience(perturbation_train_set, sample_size=batch_size)   #trajectory = weighted_sample(train_set, prob_dist)    #sample(train_set) 
				actor_loss, alpha_loss, critic1_loss, critic2_loss, cql1_scaled_loss, cql2_scaled_loss, total_c1_loss, total_c2_loss, alpha, log_pi= policy.train(experience, batch_size = batch_size)	#trajectory
				alpha_losses.append(alpha_loss.numpy())
				critic1_losses.append(critic1_loss.numpy()) # + cql1_scaled_loss)
				critic2_losses.append(critic2_loss.numpy()) # + cql2_scaled_loss)
				...
				if (i%5000 == 0 and i<10000) or i%50000 == 0:
					plot_action_animated_learnt_policy(perturbation_train_set, policy, max_states, min_states, max_actions, min_actions,"CQL_SAC", data_folder)	 #mean_states, std_states, mean_actions, std_actions, 
					evaluation_loss = evaluate_close_to_human_behaviour(perturbation_test_set, policy, max_states, min_states, max_actions, min_actions, "CQL_SAC", action_dim, data_folder)
					evaluation_losses = np.append(evaluation_losses, evaluation_loss.cpu().detach().numpy())
					policy.save('models/CQL_SAC'+data_folder+'_perturbations/CQL_SAC_policy')

				if (i%10 == 0 and i<500) or (i%50 == 0 and i<2000) or (i%200 == 0 and i<10000) or i%2000 == 0:
					ind = perturbation_test_set['trial'].unique()[42]
					states, actions, new_states, reward_, terminals= get_trajectory(perturbation_test_set, ind)
					target_corner = states[3,9:12].cpu().detach().numpy()*(max_states[9:12]-min_states[9:12]) + min_states[9:12]	#*std_states[9:12] + mean_states[9:12])		
					target_corner_angle = np.rad2deg(np.arctan2(target_corner[2], target_corner[0]))
					...
					avg_rewards, avg_hard_rewards, avg_rewards_human, avg_rewards_h, avg_hard_rewards_h, avg_losses_2_behaviour = evaluate_agent_performance(perturbation_test_set, policy, max_states, min_states, max_actions, min_actions, agent, perturbation=True)	#mean_states, std_states, mean_actions, std_actions,
					log_dict2 = {
					"AVG reward Agent": avg_rewards,
					"AVG hard reward Agent": avg_hard_rewards,
					"AVG reward Human control": avg_rewards_h,
			...
			plt.savefig('training_plots/CQL_SAC'+data_folder+'_perturbations/alpha_losses_training_curve.png')

			_, critic1_loss_curve = plt.subplots()
			critic1_loss_curve.plot(critic1_losses)
			plt.savefig('training_plots/CQL_SAC'+data_folder+'_perturbations/critic1_losses_training_curve.png')

			_, critic2_loss_curve = plt.subplots()
			critic2_loss_curve.plot(critic2_losses)
			plt.savefig('training_plots/CQL_SAC'+data_folder+'_perturbations/critic2_losses_training_curve.png')

			_, actor_loss_curve = plt.subplots()
			actor_loss_curve.plot(actor_losses)
			plt.savefig('training_plots/CQL_SAC'+data_folder+'_perturbations/actor_losses_training_curve.png')

			_, evaluation_loss_curve = plt.subplots()
			evaluation_loss_curve.plot(evaluation_losses)
			plt.savefig('training_plots/CQL_SAC'+data_folder+'_perturbations/evaluation_losses_during_training.png')

			_, cql1_scaled_loss_curve = plt.subplots()
			cql1_scaled_loss_curve.plot(cql1_scaled_losses)
			plt.savefig('training_plots/CQL_SAC'+data_folder+'_perturbations/cql1_scaled_losses_during_training.png')
			
			_, cql2_scaled_loss_curve = plt.subplots()
			cql2_scaled_loss_curve.plot(cql2_scaled_losses)
			plt.savefig('training_plots/CQL_SAC'+data_folder+'_perturbations/cql2_scaled_losses_during_training.png')"""

		else:
			print("load trained model")
			policy.load('models/CQL_SAC'+data_folder+'_perturbations/CQL_SAC_policy')
			plot_action_animated_learnt_policy(train_set, policy, max_states, min_states, max_actions, min_actions, "CQL_SAC", data_folder)	#mean_states, std_states, mean_actions, std_actions, 
			evaluation_loss = evaluate_close_to_human_behaviour(test_set, policy, max_states, min_states, max_actions, min_actions, "CQL_SAC", action_dim, data_folder)	#,mean_states, std_states, mean_actions, std_actions
			print("Evaluation loss: ", evaluation_loss)		
			avg_reward, avg_hard_reward, avg_reward_human, avg_reward_h, avg_hard_reward_h, avg_loss_2_behaviour = evaluate_agent_performance(test_set, policy, max_states, min_states, max_actions, min_actions, agent)	#mean_states, std_states, mean_actions, std_actions, 
		...
		if train == True:
			...
			wandb.init(project="reward-learning",name="TD3-5t_perturbations", config=args)
			# Magic
			wandb.watch(policy, log_freq=500)
			...
			for i in tqdm(range(1, epochs_warmup)):
				...
				if (i%5000 == 0 and i<10000) or i%50000 == 0:
					plot_action_animated_learnt_policy(train_set, policy, max_states, min_states, max_actions, min_actions, agent, data_folder)	#mean_states, std_states, mean_actions, std_actions,

					evaluation_loss = evaluate_close_to_human_behaviour(test_set, policy, max_states, min_states, max_actions, min_actions, agent, action_dim, data_folder)	#mean_states, std_states, mean_actions, std_actions,
					evaluation_losses = np.append(evaluation_losses, evaluation_loss.detach().numpy())
					policy.save('models/TD3_BC'+data_folder+'_base/TD3_BC_policy')
				...
				if (i%1000 == 0 and i<10000) or i%5000 == 0:
					...
					avg_rewards, avg_hard_rewards, avg_rewards_human, avg_rewards_h, avg_hard_rewards_h, avg_losses_2_behaviour = evaluate_agent_performance(test_set, policy, max_states, min_states, max_actions, min_actions, agent, perturbation=True)	# mean_states, std_states, mean_actions, std_actions,
					log_dict2 = {
					"AVG reward Agent": avg_rewards,
					"AVG reward Human control":avg_rewards_h,
					"AVG reward Human": avg_rewards_human,
			...
			for i in tqdm(range(1, epochs)):
				experience = weighted_sample_experience(perturbation_train_set, sample_size=batch_size)    #weighted_sample(dataset, prob_dist) #sample(dataset) 	  
				c_loss, a_loss = policy.train(experience, batch_size = batch_size) #replay_buffer #, args, **kwargs
				if a_loss != 0.0:	#actor loss updated only once every "policy_freq" update, set to 0 otherwise
					a_losses.append(a_loss.numpy())
					actor_loss = a_loss
				...
				if (i%5000 == 0 and i<10000) or i%50000 == 0:
					plot_action_animated_learnt_policy(perturbation_train_set, policy, max_states, min_states, max_actions, min_actions, agent, data_folder)	#, mean_states, std_states, mean_actions, std_actions,

					evaluation_loss = evaluate_close_to_human_behaviour(perturbation_test_set, policy, max_states, min_states, max_actions, min_actions, agent, action_dim, data_folder)	#mean_states, std_states, mean_actions, std_actions, 
					evaluation_losses = np.append(evaluation_losses, evaluation_loss.detach().numpy())
					policy.save('models/TD3_BC'+data_folder+'_perturbations/TD3_BC_policy')
			...
			_, evaluation_loss_curve = plt.subplots()
			evaluation_loss_curve.plot(evaluation_losses)
			plt.savefig('training_plots/TD3_BC'+data_folder+'_perturbations/evaluation_losses_during_training.png')

			_, actor_loss_curve = plt.subplots()
			actor_loss_curve.plot(a_losses)
			plt.savefig('training_plots/TD3_BC'+data_folder+'_perturbations/actor_losses_training_curve.png')

			_, critic_loss_curve = plt.subplots()
			critic_loss_curve.plot(c_losses)
			plt.savefig('training_plots/TD3_BC'+data_folder+'_perturbations/critic_losses_training_curve.png')

		else:
			print("load trained model")
			policy.load('models/TD3_BC'+data_folder+'_perturbations/TD3_BC_policy')
			plot_action_animated_learnt_policy(train_set, policy, max_states, min_states, max_actions, min_actions, agent, data_folder)	#mean_states, std_states, mean_actions, std_actions, 
			evaluation_loss = evaluate_close_to_human_behaviour(test_set, policy, max_states, min_states, max_actions, min_actions, agent, action_dim, data_folder)	#mean_states, std_states, mean_actions, std_actions, 
			print("Evaluation loss: ", evaluation_loss)	
			avg_rewards, avg_hard_rewards, avg_rewards_human, avg_rewards_h, avg_hard_rewards_h, avg_losses_2_behaviour = evaluate_agent_performance(perturbation_test_set, policy, max_states, min_states, max_actions, min_actions, agent, perturbation=True)	#mean_states, std_states, mean_actions, std_actions, 
			print(avg_rewards, avg_rewards_human, avg_losses_2_behaviour)
	else:
		print(agent, "is not implemented")
		policy=None
	return policy
######################### Evaluation ####################################
def evaluate_agent_performance(dataset, policy, max_states, min_states, max_actions, min_actions, agent="TD3_BC", perturbation = False):	#mean_states, std_states, mean_actions, std_actions,
	rewards = []
	hard_rewards = []
	rewards_h = []
	hard_rewards_h = []
	...
	for i, trial in tqdm(enumerate(dataset['trial'].unique())):
		...
		action_h = action_h.cpu().detach().numpy()*(max_actions-min_actions) + min_actions	#*std_actions + mean_actions
		state = states[0][:]
		...
		target_corner = state[9:12]*(max_states[9:12]-min_states[9:12]) + min_states[9:12]	#*std_states[9:12] + mean_states[9:12])		
		target_corner_angle = np.rad2deg(np.arctan2(target_corner[2], target_corner[0]))
		
		angle = np.rad2deg(np.arctan2(state[20]*(max_states[20]-min_states[20]) + min_states[20], state[18]*(max_states[18]-min_states[18]) + min_states[18]))	#*std_states[20] + mean_states[20]	*std_states[18] + mean_states[18]
		angle_h = action_h[0,0]
		dt = 0.0111
		cuevel = ((states[1][[13,14,15]].cpu().detach().numpy()*(max_states[12:15]-min_states[12:15]) + min_states[12:15])-(states[0][[13,14,15]].cpu().detach().numpy()*(max_states[12:15]-min_states[12:15]) + min_states[12:15]))/dt	#*std_states[12:15] + mean_states[12:15]	*std_states[12:15] + mean_states[12:15]
		force = np.linalg.norm(cuevel) 
		...
		for k in range(1,N):
			...
			state_cue_front, state_cue_back, state_cue_direction, angle, force = update_cuepos(state_cue_front*(max_states[12:15]-min_states[12:15]) + min_states[12:15], actions, angle, force, call="evaluation")	#*std_states[12:15] + mean_states[12:15]
			
			states[k][12:15] = torch.tensor(state_cue_front - min_states[12:15]/(max_states[12:15]-min_states[12:15]) , device=device)	 #- mean_states[12:15] / std_states[12:15] 
			states[k][15:18] = torch.tensor(state_cue_back - min_states[15:18]/(max_states[15:18]-min_states[15:18]), device=device)	#- mean_states[15:18] / std_states[15:18] 
			states[k][18:21] = torch.tensor(state_cue_direction - min_states[18:21]/(max_states[18:21]-min_states[18:21]) , device=device)	#- mean_states[18:21] / std_states[18:21]
			#states[k][21:] = actions[1:]
			if agent == "CQL_SAC":
				actions = policy.select_action(states[k,:], eval=True).cpu().detach().numpy()*(max_actions-min_actions) + min_actions	#*std_actions + mean_actions
			else:
				actions = policy.select_action(states[k,:]).cpu().detach().numpy()*(max_actions-min_actions) + min_actions	#*std_actions + mean_actions
			next_angle = actions[0]
			if k < N-1:
				next_angle_h = action_h[k+1,0]

			if k > (hit_ind-1) and k < (hit_ind+1):
				losses_2_behaviour_.append(loss_2_behaviour(angle, states[k][18:21].cpu().detach().numpy(), max_states, min_states, max_actions, min_actions))	#mean_states, std_states, mean_actions, std_actions
				if perturbation:
					reward, hard_reward = compute_reward_evaluation(prev_angle, angle, next_angle, force, corner, reward, hard_reward, max_actions, min_actions, perturbation=True)	#mean_actions, std_actions
					reward_h, hard_reward_h  = compute_reward_evaluation(prev_angle_h, angle_h, next_angle_h, force_h, corner, reward_h, hard_reward_h, max_actions, min_actions, perturbation=True)	#mean_actions, std_actions,
				else:
					reward, hard_reward  = compute_reward_evaluation(prev_angle, angle, next_angle, force, corner, reward, hard_reward, max_actions, min_actions)	#mean_actions, std_actions
					reward_h, hard_reward_h  = compute_reward_evaluation(prev_angle_h, angle_h, next_angle_h, force_h, corner, reward_h, hard_reward_h, max_actions, min_actions)	#mean_actions, std_actions

		rewards = np.append(rewards, reward)
		hard_rewards = np.append(hard_rewards, hard_reward)
		rewards_h = np.append(rewards_h, reward_h)
		hard_rewards_h = np.append(hard_rewards_h, hard_reward_h)
		rewards_human = np.append(rewards_human, reward_human)
	
	avg_rewards = np.sum(rewards)/len(rewards)
	avg_hard_rewards = np.sum(hard_rewards)/len(hard_rewards)
	avg_rewards_human = np.sum(rewards_human)/len(rewards_human)
	avg_rewards_h = np.sum(rewards_h)/len(rewards_h)
	avg_hard_rewards_h = np.sum(hard_rewards_h)/len(hard_rewards_h)
	...
	return avg_rewards, avg_hard_rewards, avg_rewards_human, avg_rewards_h, avg_hard_rewards_h, avg_losses_2_behaviour

			
def evaluate_close_to_human_behaviour(dataset, policy, max_states, min_states, max_actions, min_actions, agent="TD3_BC",  action_dim=2, data_folder=""):	#mean_states, std_states, mean_actions, std_actions,
	#losses = []
	#avg_loss = 0.0
	l=5
	...
	iteration = dataset['trial'].unique()[42]
	...
	angle_b_ = angle_b_*(max_actions[0]-min_actions[0])+ min_actions[0]	#*std_actions[0] + mean_actions[0]
	angle_e_ = angle_e_*(max_actions[0]-min_actions[0])+ min_actions[0]	#*std_actions[0] + mean_actions[0]
	angle_b_u_ = angle_b_u_*(max_actions[0]-min_actions[0])+ min_actions[0]	#*std_actions[0] + mean_actions[0]
	angle_e_u_ = angle_e_u_*(max_actions[0]-min_actions[0])+ min_actions[0]	#*std_actions[0] + mean_actions[0]
	
	magnitude_b_ = magnitude_b_*(max_actions[1]-min_actions[1])+ min_actions[1]	#*std_actions[1] + mean_actions[1]
	magnitude_e_ = magnitude_e_*(max_actions[1]-min_actions[1])+ min_actions[1]	#*std_actions[1] + mean_actions[1]
	magnitude_b_u_ = magnitude_b_u_*(max_actions[1]-min_actions[1])+ min_actions[1]	#*std_actions[1] + mean_actions[1]
	magnitude_e_u_ = magnitude_e_u_*(max_actions[1]-min_actions[1])+ min_actions[1]	#*std_actions[1] + mean_actions[1]
	...
	fig.savefig('training_plots/'+agent+data_folder+'_perturbations/iter_'+str(iteration)+'_angle_behaviour_agent.png')
	...
	fig.savefig('training_plots/'+agent+data_folder+'_perturbations/iter_'+str(iteration)+'_magnitude_behaviour_agent.png')
	fig, mean_b = plt.subplots()
	mean_b.set_xlabel("timesteps")
	...
	#fig.tight_layout()
	fig.savefig('training_plots/'+agent+data_folder+'_perturbations/iter_'+str(iteration)+'_MSE_loss_between_actions.png')

	plt.close('all')
	return loss/len(dataset["trial"])


def plot_action_animated_learnt_policy(dataset, policy, max_states, min_states, max_actions, min_actions, agent="TD3_BC", data_folder=""):	#mean_states, std_states, mean_actions, std_actions,

	iteration = dataset['trial'].unique()[42]
	states,actions_behaviour,_,_,_ = get_trajectory(dataset, dataset["trial"].iloc[iteration])
	fig, ax = plt.subplots(1,1)
	ref_cueposfront = states[:,12:15].cpu().clone().detach()*(max_states[12:15]-min_states[12:15]) + min_states[12:15]	#*std_states[12:15] + mean_states[12:15]
	ref_cueposback = states[:,15:18].cpu().clone().detach()*(max_states[15:18]-min_states[15:18]) + min_states[15:18]	#*std_states[15:18] + mean_states[15:18]
	cueposfront = np.zeros((states.shape[0],3))
	cueposback = np.zeros((states.shape[0],3))
	cuedirection = np.zeros((states.shape[0],3))

	cueposfront[0][0:3] = states[0,12:15].cpu()*(max_states[12:15]-min_states[12:15]) + min_states[12:15]	#*std_states[12:15] + mean_states[12:15]
	cueposfront[1][0:3] = states[1,12:15].cpu()*(max_states[12:15]-min_states[12:15]) + min_states[12:15]	#*std_states[12:15] + mean_states[12:15]
	cueposback[0][0:3] = states[0,15:18].cpu()*(max_states[15:18]-min_states[15:18]) + min_states[15:18]	#*std_states[15:18] + mean_states[15:18]
	cueposback[1][0:3] = states[1,15:18].cpu()*(max_states[15:18]-min_states[15:18]) + min_states[15:18]	#*std_states[15:18] + mean_states[15:18]
	cuedirection[1][0:3] = states[1,18:21].cpu()*(max_states[18:21]-min_states[18:21]) + min_states[18:21]	#*std_states[18:21] + mean_states[18:21] 
	...
	dt = 0.0111
	cuevel = ((cueposfront[1][:]*(max_states[12:15]-min_states[12:15]) + min_states[12:15]) - (cueposfront[0][:]*(max_states[12:15]-min_states[12:15]) + min_states[12:15]))/dt	#*std_states[12:15] + mean_states[12:15]	*std_states[12:15] + mean_states[12:15]
	force = np.linalg.norm(cuevel) 
	angle_.append(angle)
	force_.append(force)
	...
	for i in range(0, states.shape[0]-1):
		a = actions.detach().numpy()*(max_actions-min_actions) + min_actions	#*std_actions + mean_actions
		a_b = actions_behaviour[i,:].cpu().detach().numpy()*(max_actions-min_actions) + min_actions	#*std_actions+ mean_actions
	...
	fig.savefig('training_plots/'+agent+data_folder+'_perturbations/iter_'+str(iteration)+'_angle_animation.png')
	...
	fig2.savefig('training_plots/'+agent+data_folder+'_perturbations/iter_'+str(iteration)+'_force_animation.png')
	...
	def animate(i):
		...
		x_val = states[i,0]	*(max_states[0]-min_states[0]) + min_states[0]	#*std_states[0] + mean_states[0]
		z_val = states[i,2] *(max_states[2]-min_states[2]) + min_states[2]	#*std_states[2] + mean_states[2]
		ax.scatter(x_val, z_val, s=60, facecolors='black', edgecolors='black', label = 'cueball')
		
		ax.scatter(states[i][6]*(max_states[6]-min_states[6]) + min_states[6], states[i][8]*(max_states[8]-min_states[8]) + min_states[8], s=60, facecolors='red', edgecolors='red', label = 'target ball')	#*std_states[6] + mean_states[6]	*std_states[8] + mean_states[8]
		ax.scatter(states[i][9]*(max_states[9]-min_states[9]) + min_states[9], states[i][11]*(max_states[11]-min_states[11]) + min_states[11], s=200, facecolors='none', edgecolors='green', label = 'pocket')	#*std_states[9] + mean_states[9]	*std_states[11] + mean_states[11]
	...
	plt.close()
	anim.save('training_plots/'+agent+data_folder+'_perturbations/iter_'+str(iteration)+'_Agent_learnt_policy.gif', writer='imagemagick')	
...
if __name__ == "__main__":
	...
	for i, file in enumerate(sorted(os.listdir("data"+data_folder))):
		if file.endswith("3t_data.csv"):	#left
			filename.append("data"+data_folder+"/"+file)

	policy = train(filename, state_dim=21, action_dim=2, epochs=500000, epochs_warmup=500000, batch_size=256, train=True, agent=agent, data_folder=data_folder)	#state_dim=16 #TD3_BC #set train=False to load pretrained model
